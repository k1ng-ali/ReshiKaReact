...
export default function SignUpForm({ onSubmit }) {
    ...
    const handleChange = (e) => {
        ...
        // Для email - специальная обработка
        if (name === "email") {
        ...
        // Проверка символов для nickname и password
        if (name === "nickname" || name === "password" || name === "repeatPassword") {
            isValid = regex.test(value);
        }

        // Проверка длины для nickname и password
    ...
    const validateEmail = (email) => {
        const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // Стандартная проверка email
    ...
    const getInputClass = (fieldName) => {
        ...
        if (isValid === null) return "fieldset";          // пустое поле (серый)
        return isValid ? "fieldset valid" : "fieldset invalid";  // зеленый/красный
    };
